    ...
    <Card>
      ...
      <CardContent>
        <ResponsiveContainer width="100%" height={computedHeight}>
          <RechartsBarChart data={data} layout={isHorizontal ? 'vertical' : 'horizontal'} margin={{ top: 5, right: 10, left: 0, bottom: rotateLabels ? 80 : 5 }}>
            ...
            {isHorizontal ? (
              <>
                ...
                <YAxis type="category" dataKey={index} tick={{ fill: 'var(--color-on-surface-variant)', fontSize: 12 }} tickLine={false} axisLine={{ stroke: 'var(--color-outline-variant)' }} width={140} interval={0} />
              </>
            ) : (
              <>
                <XAxis dataKey={index} tick={{ fill: 'var(--color-on-surface-variant)', fontSize: 11, ...(rotateLabels ? { angle: -45, textAnchor: 'end' } : {}) }} tickLine={false} axisLine={{ stroke: 'var(--color-outline-variant)' }} interval={0} height={rotateLabels ? 80 : undefined} />
